  ...
  createModuleTypes: [
    {
      ...
      handlers: {
        ...
        async getModuleOutputs({ moduleConfig, version }) {
          const deploymentImageName = await containerHelpers.getDeploymentImageName(moduleConfig, undefined)
          const deploymentImageId = await containerHelpers.getDeploymentImageId(moduleConfig, version, undefined)

          return {
            outputs: {
              "local-image-name": await containerHelpers.getLocalImageName(moduleConfig),
              "local-image-id": await containerHelpers.getLocalImageId(moduleConfig, version),
